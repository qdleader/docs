import{_ as e,o as i,c as n,d as a}from"./app-C59QzqDT.js";const t={},l=a(`<h1 id="css-的渲染层合成是什么-浏览器如何创建新的渲染层" tabindex="-1"><a class="header-anchor" href="#css-的渲染层合成是什么-浏览器如何创建新的渲染层" aria-hidden="true">#</a> css 的渲染层合成是什么 浏览器如何创建新的渲染层</h1><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>在 DOM 树中每个节点都会对应一个渲染对象（RenderObject），当它们的渲染对象处于相同的坐标空间（z 轴空间）时，就会形成一个 RenderLayers，也就是渲染层。渲染层将保证页面元素以正确的顺序堆叠，这时候就会出现层合成（composite），从而正确处理透明元素和重叠元素的显示。对于有位置重叠的元素的页面，这个过程尤其重要，因为一旦图层的合并顺序出错，将会导致元素显示异常。
